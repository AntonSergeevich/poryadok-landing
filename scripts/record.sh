...
case "$CMD" in
  start)
    ...
    # -s 0 — пакет целиком. С обрезанным пакетом (было -s 128) нельзя
    # проверить контрольную сумму: для этого нужны все данные. Ради этой
    # проверки записываем полностью. Содержимое страниц при этом внутри
    # TLS и остаётся зашифрованным.
    nohup tcpdump -i any -nn -s 0 -W 6 -C 30 -Z root \
