...
# Файл .env проверяем сразу, а не в конце. Без него скрипт раньше
# доходил до последнего шага — с уже перенесёнными данными — и падал
# на «cp: cannot stat '.env'». Состояние после такого падения объяснить
# труднее всего: база готова, а сайт по-прежнему на SQLite.
[ -f .env ] || die "Нет файла .env в $ROOT — некуда прописывать адрес базы.
Он должен лежать рядом с manage.py; образец — .env.example."
