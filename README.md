...
landing/
  ...
  cabinet.py          кабинет: рабочий стол, заявки, заказчики, деньги,
                      проекты с этапами и переписка
  survey.py           вопросы разбора, подсчёт, выводы — одними данными
  scheme.py           схема процессов по ответам: где течёт и чем закрыть
  ...
  services/
    ...
    chat.py           переписка по проекту
    notify.py         уведомления о движении по проекту в Telegram
  ...
  management/commands/
    ...
    lead_reminders.py напоминает о заявках, по которым взяли паузу
...
**Цифры считаются в одном месте.** Лестница цен на главной, конструктор
и текст заявки берут число у одной и той же функции `constructor.estimate`.
Написанные руками, они однажды разошлись бы — и разошлись бы молча.
То же правило у числа вопросов: страницы называют его из кода, а проверка
в тестах падает, если на странице появилось другое.

**Схема процессов — то, что человек уносит.** После разбора `scheme.py`
собирает по ответам чертёж из пяти шагов пути клиента и отмечает, где
течёт. Рисует его сервер обычным SVG: лист печатают и сохраняют в PDF,
а печать не должна зависеть от того, выполнился ли скрипт. Открывается
он по метке в сессии, а не по номеру в адресе: ссылку с номером можно
переслать, а вместе с ней — чужие ответы про деньги.

**Тёмная тема — отдельная палитра, а не инверсия.** Тема берётся из
системы, своего переключателя нет. Меняются только цвета ролей; правил
вёрстки это не касается. Печать всегда возвращается на белую бумагу.

**Переписка — доказательная база, а не удобство.** В мессенджере
«я вам присылал» не значит ничего: сообщение удаляется, чат чистится,
файл теряется вместе с ним. Здесь сообщения не правятся и не удаляются
ни одной из сторон, включая исполнителя; имя автора копируется в момент
отправки; файлы лежат вместе с сообщением и его временем. В админке
раздел доступен только на чтение — и это не забытая галочка.

Скорость здесь условие, а не украшение: в чат, который думает секунду
на сообщение, перестают писать и возвращаются в мессенджер. Поэтому
сообщение показывается сразу, новые дозагружаются одним коротким
запросом, а опрос замолкает, когда вкладку не смотрят.
...
Оплаты нужны для счетов клиентам за системы. Клуб через них больше не
проходит: вступление бесплатное, платить там не за что.
...
Вступление бесплатное и бессрочное: человек заполняет форму, получает
одноразовую ссылку на вход сразу на странице, и та же ссылка приходит
из бота, если он поделится номером. Подписка заводится тарифом
«Свободный вход» — без цены и без даты конца.

Платные подписки, оформленные раньше, дохаживают свой срок: команды
напоминания и закрытия доступа работают только с ними, потому что
перебирают подписки по дате конца, а у бессрочной её нет.

---

## Закрытие доступа в клуб

Свободный вход бессрочный: закрывать нечего, и команда его не трогает —
у бессрочной подписки нет даты конца, а она перебирает только те, у кого
эта дата прошла. Команда осталась ради оплаченных подписок, оформленных,
пока клуб был платным. Ежедневный запуск на сервере делает cron:
...
- [ ] Проверить цены блоков в `landing/constructor.py`. Ориентир взят
      по рынку: студии за заказную CRM берут от 300 000 ₽, готовые
      сервисы — 400–5 000 ₽ в месяц. Пересматривать по загрузке.
- [ ] Настроить почту (`EMAIL_HOST` и остальное в `.env`) — без неё
      заказчик не восстановит пароль сам, и каждый такой случай будет
      звонком вам.
- [ ] Добавить `media/` в резервное копирование: база без вложений —
      это переписка со следами вместо файлов.
