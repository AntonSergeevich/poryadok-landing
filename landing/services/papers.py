...
def scope_from(project):
    ...
    Берётся из этапов, а не пишется заново: этапы уже описывают, что
    делается, и второй список того же самого разошёлся бы с первым
    в первую же правку. Строку правят руками перед выставлением —
    это черновик, а не приговор.
    """
    rows = []
