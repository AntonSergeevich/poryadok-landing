...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            ...
            'с вашим названием и суммой 10 ₽.\n'
            '\nПЛАТИТЬ НЕ НУЖНО. Этот заказ существует только на стороне\n'
            'GetPlatinum, в нашей базе его нет — доступ в клуб по нему\n'
            'НЕ ВЫДАСТСЯ, даже если оплатить. Проверяется только то, что\n'
            'ссылка создаётся и форма открывается.\n'
            '\nЧтобы проверить всю цепочку с выдачей доступа, есть --full:\n'
            'там заводится настоящая подписка, тоже на 10 ₽.\n')
